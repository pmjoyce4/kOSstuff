...
function node_apsis {
	...
	local rab is 1 / ra.
	local v1 is sqrt( vom:sqrmagnitude + 2*mu*(rab - rb) ). 	// velocity at burn apsis
	...
	local v2 is sqrt( vom:sqrmagnitude + (mu * (2/r2 - rc + 1/sma1 - 1/sma2 ) ) ).
	
	// create node
	local deltav is v2 - v1.
	
	if burnApsis:tolower = "periapsis" {
...
function node_hoh {
  runoncepath("utillib").
	...
	function hohmannDv {
		return orbital_dv(orbital_velocity(ship), orbital_velocity(target)).
	}
